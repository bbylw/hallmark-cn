...
const ROWS: CropRow[] = [
  ...
  },
  {
    ...
    days: '秋播过冬',
    type: '主粮 / 优质高筋',
    mechanism: '分蘖节位于地下 3cm 保护层，受雪盖绝热保护',
...
const PHENOLOGY_TERMS = [
  ...
  { term: '秋分', date: '9月22日', temp: '+3°C', action: '冬小麦与冬豌豆下种压实', status: '监测 5cm 土温破 10°C' },
  { term: '寒露', date: '10月8日', temp: '-2°C', action: '初霜降临，出苗齐扎根深', status: '幼苗启动抗冻糖化积累' },
  { term: '霜降', date: '10月23日', temp: '-6°C', action: '鲜叶类收尾采收，搭设风障', status: '地表形成 3cm 浅冻层' },
  { term: '立冬', date: '11月7日', temp: '-12°C', action: '浇封冻水，平铺碎秸秆覆盖', status: '冻土层向下延伸至 15cm' },
  { term: '小雪', date: '11月22日', temp: '-18°C', action: '首场暴雪固土，巡视防风栏', status: '积雪沉降 10cm 形成保温被' },
  { term: '冬至', date: '12月21日', temp: '-26°C', action: '休眠期完全封锁，禁动土表', status: '极低温考验，分蘖节休眠' },
  { term: '小寒', date: '1月5日', temp: '-30°C', action: '积雪深达 35cm，巡检野生动物', status: '雪下土温稳定维持在 -4°C' },
...
const ANATOMY_MODELS = [
  ...
  },
  {
    ...
    desc: '新雪密度仅 0.08~0.15 g/cm³，内部滞留超过 85% 静止干燥空气，导热系数低至 0.035 W/(m·K)，是天然卓越绝热层。当外界寒流骤降至 -30°C 时，20cm 连续雪盖下的土壤表层温度仍可稳定锚定在 -4°C 至 -2°C 之间。',
    metrics: ['热阻 R-Value: 3.4 m²·K/W', '内外极限温差: 24°C', '风蚀减免率: 100%'],
  },
  {
    id: 'root',
    title: '深层纵深主根锚定 (Deep Perennial Root Anchorage)',
    desc: '高寒越冬作物主根具备垂直向下的强向地性生长度，能在秋播期穿透 40cm 表层，将储藏根稳固锚定在未冻透的潜水层上方。即使地表表土冻胀龟裂，也不会发生断根拔苗现象。',
    metrics: ['主根入土深度: 85~140cm', '抗拉伸拔断应力: 18.2 N', '根冠比: 1:3.2'],
  },
  {
    id: 'cuticle',
      ...
      <div className="mx-auto max-w-(--page-max) min-w-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-rule pb-3 font-mono text-xs text-muted">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
            ...
            <span>无暖棚抗冻驯化</span>
            <span className="text-rule-dark">/</span>
            <span className="text-ink font-semibold">原生品系: 178种</span>
            <span className="text-rule-dark">/</span>
        ...
        <div className="mt-8 flex flex-wrap items-end justify-between gap-6 border-b border-rule pb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 text-right">
            <div className="rounded-xl border border-rule bg-paper-2/80 p-4 text-left font-mono">
              <div className="text-[11px] text-muted uppercase">安全越冬适栽率</div>
              <div className="mt-1 text-2xl font-bold text-accent-line">{safePercentage}%</div>
              <div className="text-[11px] text-muted">当前温度下 {safeCropsCount} / {ROWS.length} 作物露地成活</div>
            </div>
            <div className="rounded-xl border border-rule bg-paper-2/80 p-4 text-left font-mono">
              <div className="text-[11px] text-muted uppercase">冻土深度估测</div>
              <div className="mt-1 text-2xl font-bold text-ink">{frostDepthCm} cm</div>
              <div className="text-[11px] text-muted">地表 -30cm 蓄水未冻层</div>
            </div>
          </div>
        </div>
            ...
            <div className="rounded-xl border border-rule bg-paper/80 p-6 sm:p-8 shadow-sm">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-3 border-t border-rule/60 pt-6">
                <div className="rounded-lg border border-rule bg-paper p-3.5">
                  <div className="font-mono text-[11px] text-muted">预计表土冻结深度</div>
                  <div className="mt-1 font-mono text-lg font-bold text-ink">{frostDepthCm} 厘米</div>
                  <div className="mt-0.5 text-[11px] text-muted">深根系耐寒品种可穿透此深度</div>
                </div>
                <div className="rounded-lg border border-rule bg-paper p-3.5">
                  <div className="font-mono text-[11px] text-muted">20cm积雪覆盖下根区地温</div>
                  ...
                  <div className="mt-0.5 text-[11px] text-muted">雪被蜂窝孔隙有效隔绝冷空气</div>
                </div>
                <div className="rounded-lg border border-rule bg-paper p-3.5">
                  <div className="font-mono text-[11px] text-muted">露地安全过冬推荐方案</div>
                  <div className="mt-1 font-mono text-xs font-bold text-ink">
                    {localTemp <= -20 ? '覆碎秸秆 + 留出雪凹槽' : localTemp <= -10 ? '自然降雪覆盖 + 防风障' : '无须额外防寒措施'}
                  </div>
                  <div className="mt-0.5 text-[11px] text-muted">顺应物候，无人工能耗</div>
                </div>
              </div>
            </div>
              ...
              <div className="mt-6 relative">
                ...
                <div className="relative h-5">
                  {BANDS.map((b) => (
                    <span
                      key={b.label}
                      className="absolute hidden -translate-x-1/2 font-mono text-[11px] sm:block text-muted"
                        ...
                        <li key={r.code} className="transition-colors">
                          ...
                          >
                            ...
                            <div className="min-w-[140px] flex-1">
                              ...
                              <span className="block font-mono text-[11px] text-muted italic hidden sm:block">
                                {r.scientific}
                              </span>
                            </div>
                            ...
                            <span className="relative block h-5 basis-full sm:basis-auto sm:w-36 lg:w-48">
                              <span
                                ...
                                style={{
                                  left: `${pos(r.min)}%`,
                                  right: `${100 - pos(0)}%`,
                                  backgroundColor: isSafe ? 'var(--hm-accent-line)' : 'var(--hm-rule-dark)',
                                }}
                              />
                              <span
                          ...
                          {/* 展开的深度植物学档案 */}
                          {on && (
                            <div className="my-2 rounded-xl border border-rule bg-paper p-5 sm:ml-16 shadow-sm">
                              <div className="grid gap-4 sm:grid-cols-2">
                                <div>
                                  <div className="font-mono text-xs font-bold text-accent-line">抗冻生理机制</div>
                                  <p className="mt-1 text-xs sm:text-sm text-ink-2" style={{ lineHeight: 'var(--lh-relaxed)' }}>
                                    {r.mechanism}
                                  </p>
                                </div>
                                <div>
                              ...
                              <div className="mt-3 border-t border-rule/50 pt-2 font-mono text-[11px] text-muted">
        ...
        {/* 选项卡 2：二十四节气高寒物候转盘 */}
        {activeTab === 'phenology' && (
          <div className="mt-6 rounded-xl border border-rule bg-paper/80 p-6 sm:p-8 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-rule/60 pb-4">
              <div>
                <span className="font-mono text-xs font-bold tracking-wider text-accent-line uppercase">
            ...
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-5 gap-2.5">
              {PHENOLOGY_TERMS.map((term, idx) => (
                ...
                >
                  ...
                  <div className={`mt-1 font-mono text-[11px] ${selectedTerm === idx ? 'text-paper/70' : 'text-muted'}`}>
                    {term.date}
                  </div>
                </button>
              ))}
            ...
            <div className="mt-8 rounded-xl border border-rule bg-paper p-6 sm:p-8">
              ...
              <div className="mt-6 grid gap-6 sm:grid-cols-2">
                <div className="rounded-lg border border-rule/60 bg-paper-2/60 p-5">
                  ...
                  <p className="text-sm text-ink-2" style={{ lineHeight: 'var(--lh-relaxed)' }}>
                    {PHENOLOGY_TERMS[selectedTerm].action}。在高纬度冻土地区，物候农时以“地温”而非日历日期为铁律。一旦表土 5cm 降温至 0°C，所有田间耕作必须彻底静止。
                  </p>
                </div>
                <div className="rounded-lg border border-rule/60 bg-paper-2/60 p-5">
                  <div className="font-mono text-xs font-bold text-ink uppercase mb-2">
        ...
        {activeTab === 'anatomy' && (
          <div className="mt-6 rounded-xl border border-rule bg-paper/80 p-6 sm:p-8 shadow-sm">
            ...
            <div className="mt-8 grid gap-8 lg:grid-cols-12 items-center">
              {/* 左侧：物理机制解构图卡 */}
              <div className="lg:col-span-6 rounded-xl border border-rule bg-paper-2/70 p-6 sm:p-8">
              ...
              {/* 右侧：解构模型剖切可视化卡片 */}
              <div className="lg:col-span-6 rounded-xl border border-dashed border-rule bg-paper-2/40 p-6 sm:p-8 font-mono">
                <div className="text-xs font-bold text-ink uppercase mb-4">
                  越冬切面层级模拟 (From Atmosphere to Deep Root)
                </div>
                <div className="space-y-3">
                  <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/10 p-3">
                    <div className="flex justify-between text-xs font-bold text-cyan-800 dark:text-cyan-200">
                      <span>1. 大气冷风层 (Atmosphere)</span>
                      <span>极温 -30°C ~ -35°C</span>
                    </div>
                    <div className="text-[11px] text-muted mt-1">干冷大风，蒸发需求极强，气孔全闭锁</div>
                  </div>

                  <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-3">
                    <div className="flex justify-between text-xs font-bold text-blue-800 dark:text-blue-200">
                      <span>2. 纯白雪被层 (Snow Cover 20cm)</span>
                      <span>气孔多孔保温 W/(m·K) 0.035</span>
                    </div>
                    <div className="text-[11px] text-muted mt-1">截留地表逸散辐射热，温度衰减陡降 20°C</div>
                  </div>
                  ...
                  <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3">
                    ...
                    <div className="text-[11px] text-muted mt-1">浓缩葡萄糖过冷保护，分生组织完全存活</div>
                  </div>

                  <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3">
                    <div className="flex justify-between text-xs font-bold text-emerald-800 dark:text-emerald-200">
                      <span>4. 深层潜水恒温层 (Deep Earth 1.2m)</span>
                      <span>恒定温度 +2°C ~ +4°C</span>
                    </div>
                    <div className="text-[11px] text-muted mt-1">未冻结地热滋养深根系，维持微弱营养循环</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-rule bg-paper/80 p-6 sm:p-8 shadow-sm">
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-3">
                {[
                  {
                    title: '播种适期与地温标尺',
                    desc: '严禁按公历日历盲播。每日正午监测土表下 5cm 深度地温，连续三天稳定跌破 10°C 方可播种冬作。过早播种导致秋季幼苗疯长拔节易受冻，过迟播种根系发育不足难以扎牢。',
                  },
                  {
                    title: '间距行道与积雪凹槽',
                    desc: '高寒地块起垄行距设定为 40 厘米。深开垄沟，促使冬季大风将新雪自动吹填堆积在作物根部凹槽内，形成至少 15cm 以上厚度的天然避风积雪保温带。',
                  },
                  {
                    title: '霜打糖化与采收时机',
                  ...
                  <div key={idx} className="rounded-lg border border-rule bg-paper p-5">
                    <div className="font-mono text-xs font-bold text-ink uppercase mb-2">
                      法则 {idx + 1} · {item.title}
                    </div>
                    <p className="text-sm text-ink-2" style={{ lineHeight: 'var(--lh-relaxed)' }}>
