...
async function runAudit() {
  console.log('🚀 Starting Audit for StudioPage: ' + TARGET_URL)
  ...
  const firstTile = page.locator('figure[role="button"]').first()
  await firstTile.click()
  await page.waitForTimeout(150)

  const hasScope = await page.locator('text=PROJECT SCOPE').count()
  console.log(`[Interaction] Project scope inspector opened: ${hasScope > 0 ? 'PASS' : 'FAIL'}`)
  if (hasScope === 0) throw new Error('Project scope drawer failed to open on tile click')
  ...
  console.log('✅ StudioPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
