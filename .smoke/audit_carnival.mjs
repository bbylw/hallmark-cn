...
async function runAudit() {
  console.log('🚀 Starting Audit for CarnivalPage: ' + TARGET_URL)
  ...
  const isPlayingText = await page.locator('text=暂停唱针').count()
  console.log(`[Interaction] Turntable needle dropped & playing: ${isPlayingText > 0 ? 'PASS' : 'FAIL'}`)
  if (isPlayingText === 0) throw new Error('Turntable playback toggle failed')

  // 4. 测试 Tab 切换（按风格）
  const genreTab = page.locator('button:has-text("按风格")')
  await genreTab.click()
  await page.waitForTimeout(150)
  const hasGenreTitle = await page.locator('text=后摇').count()
  console.log(`[Interaction] Genre tab switched successfully: ${hasGenreTitle > 0 ? 'PASS' : 'FAIL'}`)
  if (hasGenreTitle === 0) throw new Error('Genre tab failed to render')
  ...
  console.log('✅ CarnivalPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
