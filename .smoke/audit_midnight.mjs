...
const TARGET_URL = 'https://hallmark-cn.localhost/themes/midnight'

async function runAudit() {
  console.log('🚀 Starting Audit for MidnightPage: ' + TARGET_URL)
  ...
  const p50Btn = page.locator('button:has-text("P50")')
  await p50Btn.click()
  await page.waitForTimeout(150)

  const hasP50Stat = await page.locator('h1:has-text("26ms")').count()
  console.log(`[Interaction] Switched to P50 (26ms): ${hasP50Stat > 0 ? 'PASS' : 'FAIL'}`)
  if (hasP50Stat === 0) throw new Error('Percentile P50 update failed')
  ...
  console.log('✅ MidnightPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
