...
async function capture() {
  ...
  const desktopPath = path.join(ARTIFACTS_DIR, 'terminal_desktop.png')
  ...
  await page.goto(TARGET_URL, { waitUntil: 'networkidle' })
  await page.waitForTimeout(500)
  const firstRowMobile = page.locator('div[role="button"]').first()
  if (await firstRowMobile.count() > 0) {
    await firstRowMobile.click()
    await page.waitForTimeout(200)
  }

  const mobilePath = path.join(ARTIFACTS_DIR, 'terminal_mobile.png')
