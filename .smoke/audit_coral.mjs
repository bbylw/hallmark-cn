...
async function runAudit() {
  console.log('🚀 Starting Audit for CoralPage: ' + TARGET_URL)
  ...
  const resolveBtn = page.locator('button:has-text("一键确认为账户管理费")')
  if (await resolveBtn.count() > 0) {
    await resolveBtn.click()
    await page.waitForTimeout(150)
    const hasZeroDiff = await page.locator('text=未决差额：¥ 0.00').count()
    console.log(`[Interaction] Discrepancy resolved to 0.00: ${hasZeroDiff > 0 ? 'PASS' : 'FAIL'}`)
    if (hasZeroDiff === 0) throw new Error('Ledger zero discrepancy update failed')
  }
  ...
  console.log('✅ CoralPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
