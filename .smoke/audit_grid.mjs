...
async function runAudit() {
  console.log('🚀 Starting Audit for GridPage: ' + TARGET_URL)
  ...
  // 3. 测试 CMYK 分色通道交互与错位按钮
  await page.setViewportSize({ width: 1280, height: 800 })
  const misregBtn = page.locator('button:has-text("模拟 1mm 套准微错位")')
  await misregBtn.click()
  await page.waitForTimeout(200)

  const isMisregOn = await page.locator('button:has-text("✓ 已开启 1mm 套准微错位")').count()
  console.log(`[Interaction] Misregistration toggled: ${isMisregOn === 1 ? 'PASS' : 'FAIL'}`)
  if (isMisregOn !== 1) throw new Error('Misregistration toggle button failed')

  // 点击青色通道关闭
  const cyanBtn = page.locator('button[aria-label="切换 青 色通道"]')
  await cyanBtn.click()
  await page.waitForTimeout(100)
  console.log('[Interaction] Toggled Cyan ink channel successfully')
  ...
  console.log('✅ GridPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
