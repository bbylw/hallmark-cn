...
async function runAudit() {
  console.log('🚀 Starting Audit for GardenPage: ' + TARGET_URL)
  ...
  const juneBtn = page.locator('button[aria-label*="六月"]')
  await juneBtn.click()
  await page.waitForTimeout(150)

  const hasJuneDetail = await page.locator('text=夏初荆条').count()
  console.log(`[Interaction] Switched to June detail: ${hasJuneDetail > 0 ? 'PASS' : 'FAIL'}`)
  if (hasJuneDetail === 0) throw new Error('June calendar detail update failed')
  ...
  console.log('✅ GardenPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
