...
async function capture() {
  ...
  // 切换到解剖视图测试交互丰富度
  const anatomyTab = pageDesktop.locator('button:has-text("2. 字形骨架解剖仪")')
  if (await anatomyTab.count() > 0) {
  ...
  const desktopPath = path.join(ARTIFACTS_DIR, 'hum_desktop.png')
  ...
  const mobilePath = path.join(ARTIFACTS_DIR, 'hum_mobile.png')
