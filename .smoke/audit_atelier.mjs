...
async function runAudit() {
  console.log('🚀 Starting Audit for AtelierPage: ' + TARGET_URL)
  ...
  const woolBtn = page.locator('button:has-text("美利奴羊毛")')
  await woolBtn.click()
  await page.waitForTimeout(150)

  const hasWoolSpec = await page.locator('text=密织高捻美利奴羊毛').count()
  console.log(`[Interaction] Switched to wool fabric spec: ${hasWoolSpec > 0 ? 'PASS' : 'FAIL'}`)
  if (hasWoolSpec === 0) throw new Error('Wool fabric spec update failed')
  ...
  console.log('✅ AtelierPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
