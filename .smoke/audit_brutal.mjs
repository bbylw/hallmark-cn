...
const TARGET_URL = 'https://hallmark-cn.localhost/themes/brutal'

async function runAudit() {
  console.log('🚀 Starting Audit for BrutalPage: ' + TARGET_URL)
  ...
  const solidBtn = page.locator('button:has-text("实心")')
  await solidBtn.click()
  await page.waitForTimeout(100)
  console.log('[Interaction] Switched to solid text mode successfully')

  // 4. 测试配板尺寸选择器切换
  const size825Btn = page.locator('button:has-text("8.25\\"")')
  await size825Btn.click()
  await page.waitForTimeout(100)
  const truckText = await page.locator('text=144mm 支架').count()
  console.log(`[Interaction] Deck size switch verified: ${truckText > 0 ? 'PASS' : 'FAIL'}`)
  if (truckText === 0) throw new Error('Deck size configuration update failed')
  ...
  console.log('✅ BrutalPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
