...
const TARGET_URL = 'https://hallmark-cn.localhost/themes/editorial'

async function runAudit() {
  console.log('🚀 Starting Audit for EditorialPage: ' + TARGET_URL)
  ...
  const secondArticleBtn = page.locator('section[aria-labelledby="contents"] button').first()
  await secondArticleBtn.click()
  await page.waitForTimeout(150)

  const hasExcerpt = await page.locator('text=EXCERPT').count()
  console.log(`[Interaction] Article excerpt panel opened: ${hasExcerpt > 0 ? 'PASS' : 'FAIL'}`)
  if (hasExcerpt === 0) throw new Error('Article excerpt drawer failed to render')
  ...
  console.log('✅ EditorialPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
