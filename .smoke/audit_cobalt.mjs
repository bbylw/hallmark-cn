...
async function runAudit() {
  console.log('🚀 Starting Audit for CobaltPage: ' + TARGET_URL)
  ...
  const contractBtn = page.locator('button:has-text("SLA 合同")')
  await contractBtn.click()
  await page.waitForTimeout(150)

  const hasContractDoc = await page.locator('text=master-service-agreement.pdf').count()
  console.log(`[Interaction] Sample switched to SLA contract: ${hasContractDoc > 0 ? 'PASS' : 'FAIL'}`)
  if (hasContractDoc === 0) throw new Error('Document sample switch failed')
  ...
  console.log('✅ CobaltPage Audit PASSED perfectly with 0 issues!')
  await browser.close()
}
